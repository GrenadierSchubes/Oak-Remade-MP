...
NDiplomacy = {
	...
	VOLUNTEERS_PER_COUNTRY_ARMY = 0.1,				-- Each army unit owned by the source country contributes this amount of volunteers to the limit.
...
NMilitary = {
	...
	ARMY_LEADER_XP_GAIN_PER_UNIT_IN_COMBAT = 0.01, -- XP gain per unit in combat
	CONSTANT_XP_RATIO_FOR_MULTIPLE_LEADERS_IN_SAME_COMBAT = 0.3, -- if there are multiple leaders in same combat, each one gets thisratio + 1-thisratio/num leaders. amount of xp each general gets scales 1 0.75 0.66 etc for 1 2 3 generals
	BASE_LEADER_TRAIT_GAIN_XP = 0.2,				   -- Base xp gain for traits per hour for armies.
	...
	OUT_OF_SUPPLY_SPEED = -0.35,                    -- max speed reduction from supply
...
NAir = {
	...
	COMBAT_STACK_LIMIT = 1,								-- The biggest allowed dogfight combination (1vs1 or 2vs1). Bigger value cause that amount of airplanes matters more then their stats. Only used in naval air combat, for land air combat see COMBAT_MULTIPLANE_CAP
	...
	NAVAL_STRIKE_DAMAGE_TO_STR = 1.25,					-- Balancing value to convert damage ( nanaval_strike_attackval_strike_attack * hits ) to Strength reduction.
	NAVAL_STRIKE_DAMAGE_TO_ORG = 2.5,					-- Balancing value to convert damage ( naval_strike_attack * hits ) to Organisation reduction.
	NAVAL_STRIKE_CARRIER_MULTIPLIER = 5,              -- damage bonus when planes are in naval combat where their carrier is present (and can thus sortie faster and more effectively)																																													 
...
NNavy = {
	...
	ESCAPE_SPEED_HIDDEN_SUB = 0.32,									-- hidden subs get faster escape speed
	...
	UNIT_TRANSFER_SPOTTING_SPEED_MULT = 1.5,						-- spotting speed mult against unit transfers
	NAVAL_INVASION_SPOTTING_SPEED_MULT = 1.5,						-- spotting speed mult against naval invasion armies
	...
	UNIT_TRANSFER_DETECTION_CHANCE_BASE = 15.0,							-- unit transfer and naval invasion base chance detection percentage (if this fails, no detection is done on that tick)
	BASE_SPOTTING_EFFECT_FOR_INITIAL_UNIT_TRANSFER_SPOTTING = 6.0,		-- same as BASE_SPOTTING_EFFECT_FOR_INITIAL_CONVOY_SPOTTING, but for naval transfer convoys
	SPOTTING_SPEED_EFFECT_FOR_INITIAL_UNIT_TRANSFER_SPOTTING = 15.0,	-- same as SPOTTING_SPEED_EFFECT_FOR_INITIAL_CONVOY_SPOTTING, but for naval transfer convoys
	BASE_SPOTTING_EFFECT_FOR_INITIAL_NAVAL_INVASION_SPOTTING = 0.75,		-- same as BASE_SPOTTING_EFFECT_FOR_INITIAL_CONVOY_SPOTTING, but for naval invasion convoys
	SPOTTING_SPEED_EFFECT_FOR_INITIAL_NAVAL_INVASION_SPOTTING = 0.05,	-- same as SPOTTING_SPEED_EFFECT_FOR_INITIAL_CONVOY_SPOTTING, but for naval invasion convoys
	...
	RELATIVE_SURFACE_DETECTION_TO_POSITIONING_FACTOR				= 0.00,	-- multiples the surface detection difference between two sides. the side with higher detection will get a bonus of this value
	MAX_POSITIONING_BONUS_FROM_SURFACE_DETECTION					= 0.00,  -- will clamp the bonus that you get from detection
	...
	MAX_POSITIONING_PENALTY_FROM_HIGHER_SHIP_RATIO					= 0.60,  -- maximum penalty to get from larger fleets
	...
	SCREENING_EFFICIENCY_PENALTY_ON_MINIMUM_POSITIONING				= 0.10,  -- screening efficiency (screen to capital ratio) at 0% positioning
...
NAI = {
	...
	UPGRADE_PERCENTAGE_OF_FORCES = 0.1,					-- How 	part of the army that should be considered for upgrading
